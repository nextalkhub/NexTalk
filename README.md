...
8. [Нефункциональные требования (NFR)](#8-нефункциональные-требования-nfr)
9. [Где хранятся данные](#9-где-хранятся-данные)
10. [Фронтенд](#10-фронтенд)
11. [За рамками MVP](#11-за-рамками-mvp)
...
| Функция                              | Реализация                                                                                                                                                                                                  |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Маршрутизация API <br>(для сервисов) | 1. `/api/guilds/*`, `/api/invites` - Guild Service<br>2. `/api/channels/*`, `/api/messages` - Messaging Service<br>3. `/api/voice/*` - Voice Service<br><br>`/api/` префикс стрипается перед проксированием |
| WebSocket проксирование              | `/hubs/*` - WebSocket Gateway<br><br>`/hubs/` префикс НЕ стрипается перед проксированием                                                                                                                    |
| Swagger UI                           | `/swagger` - статический HTML с CDN<br>`/swagger/{service}` - проксирует в сервис<br><br>Используется только в development среде                                                                            |
| Rate Limiting                        | `limit_req_zone` <br>- 100 RPS/IP для API<br>- 20 RPS/IP для auth                                                                                                                                           |
| Correlation ID                       | `proxy_set_header X-Request-Id $request_id`<br><br>Генерируется на nginx и передается на все сервисы                                                                                                        |
| Internal API                         | `/internal/*` - отклоняются извне, доступны только во внутренней сети                                                                                                                                       |
...
> Полностью берет на себя аутентификацию. Конфигурация создается автоматически через `zitadel-bootstrap`

| Что делает      | Как                                                                                 |
| --------------- | ----------------------------------------------------------------------------------- |
| Регистрация     | Встроенная UI-форма                                                                 |
| Логин           | OIDC Authorization Code Flow + PKCE                                                 |
| JWT выдача      | Access token с claims:<br>`sub`, `email`, `name`, `preferred_username`              |
| Refresh token   | Для его выдачи нужно запросить `offline_access` scope                               |
| User management | Zitadel Console `/ui/console`                                                       |
| Swagger OAuth2  | OIDC application "NexTalk Swagger UI".<br>redirect: `/swagger/oauth2-redirect.html` |
| Web SPA         | OIDC application "NexTalk SPA".<br>redirect: `/callback`                            |
| OIDC discovery  | `/.well-known/openid-configuration` - используется для валидации JWT бэкендом       |
...
| --------- | ------------ | --------------------------------- |
| WebSocket | `/hubs/chat` | SignalR Hub для real-time событий |
> Auth: токен передается query-параметром `?access_token=<jwt>` - браузер не поддерживает кастомные заголовки при WS-handshake

#### ChatHub - методы клиент -> сервер

| Метод         | Назначение                                                        |
| ------------- | ----------------------------------------------------------------- |
| `SendMessage` | Отправить сообщение POST `/internal/messages` в Messaging Service |
| `Heartbeat`   | Обновить presence (пользователь подтверждает, что он в онлайне)   |
#### ChatHub - события сервер -> клиент

 > Все события приходят через один метод `GatewayEvent`
 > в формате: `{"type": "<event.type>", "payload": { ...поля }}`
...
| `POST` | `/internal/disconnect/guild/{guildId}/user/{userId}` | Отключение из гильдии (кик/бан): шлет `guild.force.disconnect` пользователю + `member.left` гильдии |
...
> Все требуют `Authorization: Bearer <jwt>`. Идентификатор пользователя извлекается из JWT `sub` claim внутри сервиса - никаких дополнительных заголовков передавать не нужно.
...
> Вызываются напрямую к `guild-service:5001` из других сервисов. JWT не требуется - доверие на уровне сети (Docker/k8s). Параметры передаются через route и query string.

| Метод | Путь                                    | Назначение                                                                              |
| :---- | :-------------------------------------- | :-------------------------------------------------------------------------------------- |
| `GET` | `/internal/channels/{channelId}/access` | Проверить права. Query: `userId`. Response: `{ hasAccess, guildId, channelType, role }` |
...
### 7.2 Retry + Exponential Backoff + Jitter
...
- `DeadlineMiddleware` (Guild Service, Messaging Service): если дедлайн истек - `504 { error, retryAfter: 5 }` сразу; иначе - `CancellationTokenSource(remaining)`, связанный с `HttpContext.RequestAborted`
...
Один экземпляр Postgres 17 с двумя базами:

| База данных | Владелец | Что хранит          |
| ----------- | -------- | ------------------- |
| `zitadel`   | Zitadel  | Управляется Zitadel |
| `nextalk`   | Сервисы  | Бизнес-данные       |
...
> Каждый сервис подключается только к своей схеме

Полная карта таблиц, типов колонок, индексов, FK и связей с Zitadel `sub`:
- Описание + ER-диаграммы (Mermaid): [docs/schema.md](docs/schema.md)
- Импорт для [dbdiagram.io](https://dbdiagram.io/d): [docs/schema.dbml](docs/schema.dbml)
