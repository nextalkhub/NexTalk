...
**Схема БД:** `messaging` (`Messages`, `OutboxEvents`, `IdempotencyKeys`)  
...
| `POST` | `/internal/disconnect/{userId}` | Полное отключение пользователя: шлет `force.disconnect` |
...
WS Gateway: пользователь хочет отправить сообщение в channel X
    │
    ├──→ Guild Service: GET /internal/channels/{X}/access?userId=123
    │    Guild Service: SELECT channel JOIN members WHERE channel.id=X AND member.user_id=123
    │    Guild Service: role == 'Member' → hasAccess: true
    │    └──→ WS Gateway: { hasAccess: true, guildId: G, channelType: "text", role: "Member" }
    │
...
| `messaging` | `Messages`, `OutboxEvents`, `IdempotencyKeys` | Messaging Service |

> Имена таблиц в `guild` - snake_case (явный `ToTable("...")` в `GuildDbContext`); в `messaging` - PascalCase (EF Core по умолчанию, без `ToTable`).
