...
| POST `/internal/disconnect/guild/{guildId}/user/{userId}` | Принудительное отключение (при бане), body: `{ reason }` |
...
| GET /internal/guilds/{guildId}/access | Проверка прав (inter-service), query: `?userId={userId}` |
...
| Эндпоинт | Назначение |
|:--|:--|
| POST `/internal/broadcast/guild/{guildId}` | Принять событие от Outbox → рассылка клиентам гильда |
