...
# нагрузку даёт k6 ramp.js, а реплики добавляет сам HPA по метрикам CPU.
#
...
#
# Что проверяет:
#   - под нагрузкой HPA desired растёт, реплики догоняют (scale-up реально идёт)
#   - witness-эндпоинт остаётся доступным (рост нагрузки не ломает сервис)
...
slo "доступность witness ≥ 99%, пик реплик строго > стартовых"
...
log "Измеренная доступность witness: ${avail}% (SLO ≥ 99%)"
log "Реплики: старт ${START_REPLICAS} → пик ${PEAK_REPLICAS}"

(( avail < 99 )) && fail "доступность ${avail}% < 99% под нагрузкой"
