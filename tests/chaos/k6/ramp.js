 ...
 * растет ступенями и ДЕРЖИТСЯ на каждой, чтобы HPA успел среагировать:
 *   - scaleUp stabilizationWindow = 120s -> каждый плато ≥ 2 мин
 *   - scaleDown stabilizationWindow = 300s -> финальное затишье ≥ 5 мин
 *
 * Нагрузка идет на /api/guilds -> CPU растет у guild-service, поэтому
 * масштабируется в первую очередь его HPA (см. SC-15).
 *
 * Что смотреть в Grafana (row "Kubernetes / Scaling") во время прогона:
 *   1. RPS растет по ступеням
 *   2. HPA desired ползет вверх 2 -> ... -> max
...
export const options = {
    stages: [
        ...
        { duration: '1m', target: 60  },  // подъем
        { duration: '3m', target: 60  },  // плато: ждем scale-up (>120s окна)
        { duration: '1m', target: 120 },  // подъем к пику
    ...
    thresholds: {
        ...
        http_req_failed:   ['rate<0.02'],   // < 2% ошибок на всем прогоне
