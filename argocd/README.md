...
Сейчас деплой идёт через Ansible (`make helm`). ArgoCD планируется как замена для GitOps-подхода: авто-синхронизация чарта при пуше в main, rollback через UI.
