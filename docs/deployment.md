...
Пунктир VIP ↔ control-plane = VIP держит одну ноду в каждый момент, через ARP перехватывает другую при failover. Между control-plane нодами — etcd Raft.
...
|:------------|:-------|:----------|:--------------------------------------------------------------------------------------------------------|
| k3s control-plane | 3 | нет     | k3s server, embedded etcd, kube-vip (DaemonSet)                                                         |
...
Обоснование:
- **2/4 GB на control-plane** — [docs.k3s.io Sizing Guide](https://docs.k3s.io/installation/requirements): `2 vCPU / 4 GB → до 350 agent-нод`. Меньше — риск нестабильного etcd.
...
worker'ы и `kubectl` обращаются к `https://10.19.0.10:6443` (VIP). [kube-vip](https://kube-vip.io/docs/usage/k3s/) — static pod на каждой control-plane ноде, держит VIP и переключает за 3–5 сек при падении.
...
|:-------------------|:-----------------------------------------------------------------------------------|:----------------------------------------|
| 1 control-plane    | VIP перехвачен за 5 сек, кластер работает                                          | Авто после возврата                     |
...
| `k3s-cluster.yml`     | k3s HA install + kube-vip                               | control-plane + worker |
...
4.2. Установить k3s server на control-plane-1 с `--cluster-init`
4.3. Присоединить control-plane-2, control-plane-3 через `--server https://10.19.0.10:6443`
4.4. `kubectl get nodes` — 3 ноды с ролью `control-plane,etcd,master` (метка `master` оставлена k3s для backwards compat)
4.5. Скачать kubeconfig с control-plane-1, заменить server на VIP, положить на bastion
4.6. Проверить failover: выключить control-plane ноду, повторить `kubectl get nodes`
