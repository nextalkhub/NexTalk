...
- Разделение слоёв: stateless (k3s) / stateful (db-vps) / observability (obs-vps).
...
graph TB
    ...
    subgraph "Private Network 10.19.0.0/16"
        ...
        OBS["obs-vps 10.19.0.41<br/>Prometheus + Loki + Grafana + Jaeger"]
...
| obs-vps     | 1      | опц.      | docker-compose: Prometheus, Loki, Jaeger, Grafana, OTel Collector, Alloy                                |
...
- **40 GB на obs-vps** — требует агрессивного retention. См. [§8.3](#83-стек-на-obs-vps) и [§11.10](#1110-disk-pressure-на-obs-vps).
...
| obs-vps            | Приложение работает, теряем метрики/логи. k3s Prometheus буферизует WAL.          | Поднять obs → буфер дофорвардится       |
...
| db-vps     | `10.19.0.31`  | —            |
| obs-vps    | `10.19.0.41`  | опц. (Grafana) |
...
| obs-vps | `10.19.0.0/16`       | 4317, 4318 (OTLP), 3100 (Loki), 9090 (PG remote_write)             |
| obs-vps | `0.0.0.0/0` (опц.)   | 443 (Grafana)                                                      |
| все     | админ IP             | 22 (SSH)                                                           |

Наружу только 80/443 на workers и опц. 443 на obs для Grafana. Управление и межсервисный трафик — внутри приватной сети.
...
- **Метрики:** k3s Prometheus (retention 1d) → `remote_write` → obs-vps Prometheus (`--web.enable-remote-write-receiver`).
- **Логи:** Grafana Alloy как DaemonSet ([alloy-k8s.alloy](../charts/nextalk/files/alloy-k8s.alloy)) читает pod logs через k8s API → Loki.
- **Трейсы:** OTLP gRPC напрямую на obs-vps:4317.
...
Минимум: `pg_dump --format=custom` через cron 1 раз в сутки, копия на obs-vps по rsync, retention 7d + 4w.
...
| `obs.yml`             | docker + compose stack                                  | obs-vps            |
...
[obs]
obs-vps ansible_host=10.19.0.41
...
При падении obs-vps теряем мониторинг. k3s Prometheus буферизует remote_write в WAL короткое время. Митигация — быстрое восстановление через Ansible.
...
### 11.10 Disk pressure на obs-vps

40 GB под весь observability-стек = retention максимум 7 дней (см. [§8.3](#83-стек-на-obs-vps)). При росте трафика — диск переполняется быстро. Митигация — Loki compactor, Prometheus tsdb retention, плюс мониторинг диска.
...
7.1. Docker + Docker Compose на obs-vps
