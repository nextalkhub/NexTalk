...
### SSH
- Bastion-only через ProxyJump. 8 из 9 серверов не имеют SSH-порта в интернете.
- ⚠ Парольный root-вход временно разрешен (упрощает первый bootstrap). После успешного деплоя - отключить: `PermitRootLogin prohibit-password`, `PasswordAuthentication no`, добавить fail2ban.
...
- Image scanning: Trivy в CI (`.github/workflows/ci-cd.yml`) - SARIF в GitHub Security tab, exit-code: 1 блокирует билд на HIGH/CRITICAL с готовым фиксом (`ignore-unfixed: true`).

### Обновления
- `unattended-upgrades` включен на всех нодах, security-канал, ежедневно.
- k3s / Helm-чарт обновляются вручную через Ansible (новая итерация playbook'а).
...
### Audit
- ⚠ k3s audit log отключен. Без него инцидент через apiserver не разобрать. Следующая итерация - включить `audit-log-path`.
...
4. k3s audit log + парсинг в Loki.
5. mTLS между сервисами - когда вырастет нагрузка и появится бюджет на ресурсы.
